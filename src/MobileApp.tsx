...
import { MobileDashboardLayout } from './app/layout';
...
import {
  SchedulePage as StudentSchedulePage,
  StudentSubjectsPage,
  StudentSubjectInfoPage,
  StudentLessonsPage,
  StudentLessonFullDetailsPage,
  StudentAbsenceRequestsPage,
  StudentProfilePage,
} from './pages/dashboards/student';
import {
  SchedulePage as TeacherSchedulePage,
  SubjectsPage as TeacherSubjectsPage,
  StudentGroupsPage as TeacherStudentGroupsPage,
  GroupSubjectInfoPage as TeacherGroupSubjectInfoPage,
  AbsenceRequestsPage as TeacherAbsenceRequestsPage,
  TeacherProfilePage,
...
function TelegramBackBridge() {
  const navigate = useNavigate();
  const location = useLocation();
  const onBack = useCallback(() => navigate(-1), [navigate]);
  const segments = location.pathname.split('/').filter(Boolean);
  const enabled = segments.length > 3;
  useTelegramBackButton(location.pathname, onBack, enabled);
  return null;
}
                  ...
                  element={
                    <RequireAuth>
                      <RequireDashboardRole dashboard="admin">
                        <MobileDashboardLayout variant="admin" />
                  ...
                  element={
                    <RequireAuth>
                      <RequireDashboardRole dashboard="teacher">
                        <MobileDashboardLayout variant="teacher" />
                      </RequireDashboardRole>
                    </RequireAuth>
                  }
                >
                  ...
                  <Route path="student-groups" element={<TeacherStudentGroupsPage />} />
                  <Route path="student-groups/:groupId" element={<TeacherGroupSubjectInfoPage />} />
                  ...
                  element={
                    <RequireAuth>
                      <RequireDashboardRole dashboard="student">
                        <MobileDashboardLayout variant="student" />
                      </RequireDashboardRole>
                    </RequireAuth>
                  }
                >
                  ...
                  <Route path="subjects/:offeringId" element={<StudentSubjectInfoPage />} />
                  <Route path="lessons" element={<StudentLessonsPage />} />
                  <Route path="lessons/:lessonId" element={<StudentLessonFullDetailsPage />} />
                  <Route path="absence-requests" element={<StudentAbsenceRequestsPage />} />
                </Route>
                <Route
                  path="*"
                  element={
